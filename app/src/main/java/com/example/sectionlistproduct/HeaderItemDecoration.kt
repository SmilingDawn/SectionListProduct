...
class HeaderItemDecoration(
        ...
        parent.addOnItemTouchListener(object : RecyclerView.SimpleOnItemTouchListener() {
            override fun onInterceptTouchEvent(
                ...
                return if (motionEvent.action == MotionEvent.ACTION_DOWN) {
                    val isHeaderTouch = motionEvent.y <= (currentHeader?.second?.itemView?.bottom ?: 0)
                    if (isHeaderTouch) {
                        callAllPerformClicks(currentHeader?.second?.itemView)
                    }
                    isHeaderTouch
                } else {
                    false
    ...
    private fun callAllPerformClicks(root: View?) {
        if (root !is ViewGroup) {
            root?.performClick()
            return
        }
        for (index in 0..root.childCount) {
            val childView: View? = root.getChildAt(index)
